...
1. **Server-side scaling is linear** up to the proxy bottleneck. Each Aurora node
   adds ~16.4 rps of Llama-3-8B throughput (TP=1, 2048in/128out).

2. **LiteLLM proxy (2 workers) saturates at ~200 rps**, becoming the bottleneck
   at 16+ nodes. Scaling LiteLLM workers proportionally should extend the linear
   region.

3. **64-node test failed** — exceeded the debug-scaling queue's node limit. Requires
   the prod queue.

4. **Streaming disabled** due to LiteLLM's MidStreamFallbackError on repetitive
   model output. TTFT measurement requires either bypassing LiteLLM (dest=direct)
   or configuring LiteLLM to disable repetition detection.
