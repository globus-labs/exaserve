#!/usr/bin/env python3
"""Compare HAProxy, Direct-MPI, and Direct-Fat dispatch modes on the same plot."""
import json
import os
import sys
...
SPECS = {
    "HAProxy (1 LB on head)": ("weakscaling_haproxy_short_v3", "run1"),
    "Direct-MPI (1 client/node)": ("weakscaling_direct_short_v3", "run1"),
    "Direct-Fat (1 client on head)": ("weakscaling_direct_fat_short_v3", "run0"),
}
NODE_COUNTS = [1, 2, 4, 8, 16, 32, 64, 128, 256]


def load_one(spec_name: str, run_group: str = "run1") -> list[tuple[int, float, float, float]]:
...
def main() -> int:
    ...
    colors = {
        ...
        "Direct-MPI (1 client/node)": "#d62728",
        "Direct-Fat (1 client on head)": "#2ca02c",
    }
    markers = {
        "HAProxy (1 LB on head)": "o",
        "Direct-MPI (1 client/node)": "s",
        "Direct-Fat (1 client on head)": "D",
    }

    for label, (spec, rg) in SPECS.items():
        rows = load_one(spec, rg)
        if not rows:
    ...
    if base_rps:
        ...
        ax_rps.plot(ideal_x, ideal_y, "--", color="gray",
                    label=f"Ideal linear (= {base_rps:.0f} RPS x N)",
    ...
    fig.suptitle(
        "Three Dispatch Modes — Aurora Weak-Scaling v3 (run0)",
