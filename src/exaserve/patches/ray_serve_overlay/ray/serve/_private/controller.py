...
class ServeController:
    ...
    async def run_control_loop_step(
        ...
        import time as _ctime, os as _cos, socket as _csock, json as _cjson
        _ctick_t0 = _ctime.time()
        _csubs = {}
        _cnode_start = _ctime.time()
        try:
            self.cluster_node_info_cache.update()
        except Exception:
            logger.exception("Exception updating cluster node info cache.")
        _csubs["cluster_node_info_update"] = round(_ctime.time() - _cnode_start, 4)
        ...
        try:
            ...
            _csubs["dsm_update"] = round(time.time() - dsm_update_start_time, 4)
        ...
        try:
            ...
            _csubs["asm_update"] = round(time.time() - asm_update_start_time, 4)
        ...
        self._update_proxy_nodes()
        _csubs["node_update"] = round(time.time() - node_update_start_time, 4)
        ...
        if self.proxy_state_manager and self.done_recovering_event.is_set():
            try:
                ...
                _csubs["proxy_state_update"] = round(time.time() - proxy_update_start_time, 4)
        ...
        # ── probe: write per-tick JSONL to node-local /tmp ────────────
        # Controller runs on head; exaserve_serve's _collect_instrumentation_all
        # gathers all /tmp/exaserve_inst/* to Lustre at end of Stage 3.
        _ctick_total = round(_ctime.time() - _ctick_t0, 4)
        _csubs["total"] = _ctick_total
        _cinst_dir = "/tmp/exaserve_inst"
        try:
            _cos.makedirs(_cinst_dir, exist_ok=True)
            with open(f"{_cinst_dir}/controller_ticks_{_cos.getpid()}.jsonl", "a") as _cf:
                _cf.write(_cjson.dumps({
                    "t": _ctick_t0,
                    "num_loops": num_loops,
                    "done_recovering": self.done_recovering_event.is_set(),
                    "subs": _csubs,
                    "total_s": _ctick_total,
                }) + "\n")
        except Exception:
            pass
