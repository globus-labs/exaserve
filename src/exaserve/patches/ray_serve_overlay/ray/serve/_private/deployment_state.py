...
class DeploymentStateManager:
    ...
    def update(self) -> bool:
        ...
        """
        import time as _dtime, os as _dos, socket as _dsock, json as _djson
        _d_t0 = _dtime.time()
        _d_subs = {}
        # Count total replicas across all deployments for scaling context
        _d_n_replicas = sum(
            len(ds._replicas.get()) for ds in self._deployment_states.values()
        ) if self._deployment_states else 0
        ...
        _d_subs["s1_check_and_update_replicas"] = round(_dtime.time() - _d_t, 4)

        # STEP 2: Check current status
        _d_t = _dtime.time()
        for deployment_state in self._deployment_states.values():
            deployment_state.check_curr_status()
        _d_subs["s2_check_curr_status"] = round(_dtime.time() - _d_t, 4)
        ...
        _d_subs["s3_drain_nodes"] = round(_dtime.time() - _d_t, 4)
        ...
        _d_subs["s4_scale_replicas"] = round(_dtime.time() - _d_t, 4)
        ...
        _d_subs["s5_update_status"] = round(_dtime.time() - _d_t, 4)
        ...
        _d_subs["s6_schedule_and_stop"] = round(_dtime.time() - _d_t, 4)
        ...
        for deployment_id, deployment_state in self._deployment_states.items():
            ...
            _d_broadcasts += 1
        _d_subs["s7_broadcast"] = round(_dtime.time() - _d_t, 4)
        ...
        # ── Section 6.2 probe: log dsm sub-phase breakdown to /tmp ─────
        # exaserve_serve's _collect_instrumentation_all gathers to Lustre.
        _d_subs["total"] = round(_dtime.time() - _d_t0, 4)
        if _d_subs["total"] > 0.001:  # skip trivial ticks
            _d_inst_dir = "/tmp/exaserve_inst"
            try:
                _dos.makedirs(_d_inst_dir, exist_ok=True)
                with open(f"{_d_inst_dir}/dsm_updates_{_dos.getpid()}.jsonl", "a") as _df:
                    _df.write(_djson.dumps({
                        "t": _d_t0,
                        "n_replicas_total": _d_n_replicas,
                        "n_deployments": len(self._deployment_states),
                        "n_broadcasts_attempted": _d_broadcasts,
                        "subs": _d_subs,
                    }) + "\n")
            except Exception:
                pass
