...
class AsyncioRouter:
    ...
    def update_deployment_targets(self, deployment_target_info: DeploymentTargetInfo):
        import time as _utime, os as _uos, socket as _usock, json as _ujson
        _u_t0 = _utime.time()
        _u_n = len(deployment_target_info.running_replicas)
        ...
        _u_dur = _utime.time() - _u_t0
        # Node-local /tmp: single append per call; infrequent (1-3 per proxy
        # at 128n) so not a Lustre concern even if it were there. Kept on
        # /tmp for consistency + exaserve_serve gathers to Lustre at end.
        _u_dir = "/tmp/exaserve_inst"
        try:
            _uos.makedirs(_u_dir, exist_ok=True)
            with open(f"{_u_dir}/router_updates_{_uos.getpid()}.jsonl", "a") as _uf:
                _uf.write(_ujson.dumps({
                    "t": _u_t0,
                    "event": "update_deployment_targets",
                    "n_replicas": _u_n,
                    "duration_s": round(_u_dur, 4),
                    "pid": _uos.getpid(),
                }) + "\n")
        except Exception:
            pass
