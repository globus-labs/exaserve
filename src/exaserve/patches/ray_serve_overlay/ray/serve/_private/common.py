...
# ── Aurora probe helpers for get_actor_handle ─────────────────────────
# Previous version did open/write/close PER get_actor_handle call, which
# hammered the Lustre MDS (up to 320k opens at 128n). This version buffers
# in memory and flushes every EXASERVE_PROBE_FLUSH_N calls. Still writes to
# Lustre under $EXASERVE_RUN_LOG_DIR but with ~100× fewer open/close ops.
import atexit as _exaserve_atexit
import os as _exaserve_os
import socket as _exaserve_socket

_EXASERVE_PROBE_FLUSH_N = int(_exaserve_os.environ.get("EXASERVE_PROBE_FLUSH_N", "500"))
_exaserve_probe_buf: list[tuple[float, float]] = []
_exaserve_probe_path: Optional[str] = None
_exaserve_probe_inited = False


def _exaserve_probe_init():
    global _exaserve_probe_path, _exaserve_probe_inited
    _exaserve_probe_inited = True
    # Write to node-local tmpfs. exaserve_serve._collect_instrumentation_all
    # gathers this to Lustre at end of Stage 3 via Ray node-affinity remote
    # tasks. Keeps MDS load to ~1 open per file per node.
    inst_dir = "/tmp/exaserve_inst"
    try:
        _exaserve_os.makedirs(inst_dir, exist_ok=True)
        _exaserve_probe_path = f"{inst_dir}/get_actor_calls_{_exaserve_os.getpid()}.csv"
    except Exception:
        _exaserve_probe_path = None
    _exaserve_atexit.register(_exaserve_probe_flush)


def _exaserve_probe_flush():
    global _exaserve_probe_buf
    if not _exaserve_probe_path or not _exaserve_probe_buf:
        return
    try:
        # One fopen/fwrite/fclose for up to FLUSH_N entries.
        with open(_exaserve_probe_path, "a") as f:
            f.write("".join(f"{t:.6f},{dur*1000:.3f}\n" for (t, dur) in _exaserve_probe_buf))
        _exaserve_probe_buf.clear()
    except Exception:
        pass


def _exaserve_probe_record(t: float, dur: float):
    if not _exaserve_probe_inited:
        _exaserve_probe_init()
    if _exaserve_probe_path is None:
        return
    _exaserve_probe_buf.append((t, dur))
    if len(_exaserve_probe_buf) >= _EXASERVE_PROBE_FLUSH_N:
        _exaserve_probe_flush()
...
class RunningReplicaInfo:
    ...
    def get_actor_handle(self) -> ActorHandle:
        ...
        _g_dur = _gtime.time() - _g_t0
        # Buffered append: 1-entry-per-call without per-call fopen/fclose on
        # Lustre. _g_buf and _g_sink live at module level (_exaserve_probe
        # helpers below) and flush every FLUSH_N calls or at process exit.
        _exaserve_probe_record(_g_t0, _g_dur)
        return actor_handle
