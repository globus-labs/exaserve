...
class ProxyActor(ProxyActorInterface):
    def __init__(
        ...
        *,
        ...
        import time as _ptime, os as _pos, socket as _psock, json as _pjson
        _init_wall_start = _ptime.time()
        _host = _psock.gethostname()
        _pid = _pos.getpid()
        _substeps = {}
        print(f"[ProxyProf pid={_pid} host={_host}] __init__ START wall={_init_wall_start:.3f}", flush=True)
        ...
        _substeps["super_init"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["configure_options"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["long_poll_client"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["memory_profiler"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["logging_context"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["create_proxies"] = round(_ptime.time() - _t, 4)
        ...
        _substeps["server_tasks_and_gc"] = round(_ptime.time() - _t, 4)

        _init_wall_end = _ptime.time()
        _init_dur = round(_init_wall_end - _init_wall_start, 4)
        _substeps["total"] = _init_dur
        print(f"[ProxyProf pid={_pid} host={_host} node={self._node_id[:16]}] __init__ END wall={_init_wall_end:.3f} dur={_init_dur:.3f}s substeps={_pjson.dumps(_substeps)}", flush=True)

        # Write detailed init profile to node-local /tmp. exaserve_serve's
        # _collect_instrumentation_all gathers to Lustre at end of Stage 3
        # via a single Ray remote task per node (one Lustre write per file).
        _inst_dir = "/tmp/exaserve_inst"
        try:
            _pos.makedirs(_inst_dir, exist_ok=True)
            with open(f"{_inst_dir}/proxy_init_{_host}_{_pid}.json", "w") as _f:
                _pjson.dump({
                    "event": "proxy_init",
                    "hostname": _host,
                    "pid": _pid,
                    "node_id": self._node_id,
                    "wall_start": _init_wall_start,
                    "wall_end": _init_wall_end,
                    "duration_s": _init_dur,
                    "substeps": _substeps,
                }, _f, indent=2)
        except Exception:
            pass
    ...
    async def ready(self) -> str:
        ...
        """
        import time as _ptime, os as _pos, socket as _psock
        _ready_wall_start = _ptime.time()
        print(f"[ProxyProf pid={_pos.getpid()} host={_psock.gethostname()}] ready() START wall={_ready_wall_start:.3f}", flush=True)
        ...
        _after_http = _ptime.time()
        print(f"[ProxyProf pid={_pos.getpid()}] ready() http_server done +{_after_http - _ready_wall_start:.3f}s", flush=True)
        ...
        _ready_wall_end = _ptime.time()
        _ready_dur = round(_ready_wall_end - _ready_wall_start, 4)
        print(f"[ProxyProf pid={_pos.getpid()}] ready() END wall={_ready_wall_end:.3f} dur={_ready_dur:.3f}s http={_after_http - _ready_wall_start:.3f}s", flush=True)

        # Append ready timing to the same /tmp file we wrote in __init__.
        _inst_dir = "/tmp/exaserve_inst"
        try:
            import json as _rjson
            _prof_path = f"{_inst_dir}/proxy_init_{_psock.gethostname()}_{_pos.getpid()}.json"
            if _pos.path.exists(_prof_path):
                with open(_prof_path) as _rf:
                    _data = _rjson.load(_rf)
                _data["ready_wall_start"] = _ready_wall_start
                _data["ready_wall_end"] = _ready_wall_end
                _data["ready_duration_s"] = _ready_dur
                _data["ready_http_s"] = round(_after_http - _ready_wall_start, 4)
                with open(_prof_path, "w") as _wf:
                    _rjson.dump(_data, _wf, indent=2)
        except Exception:
            pass
