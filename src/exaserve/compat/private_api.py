...
PRIVATE_SURFACE: tuple[PrivateSymbol, ...] = (
    ...
    PrivateSymbol(
        "serve_long_poll_client",
        "ray.serve._private.long_poll",
        "LongPollClient",
        "Serve exposes replica-set and route changes only through its internal "
        "controller long-poll stream; periodic serve.status() fleet scans are "
        "forbidden by the production readiness contract.",
    ),
    PrivateSymbol(
        "serve_long_poll_namespace",
        "ray.serve._private.long_poll",
        "LongPollNamespace",
        "The pinned controller event stream requires Serve's internal typed "
        "namespace keys; there is no public subscription API in Ray 2.53.",
    ),
    PrivateSymbol(
        "serve_deployment_id",
        "ray.serve._private.common",
        "DeploymentID",
        "Replica-set event keys use Serve's application-scoped DeploymentID; "
        "plain strings cannot subscribe to multi-application deployments.",
    ),
    PrivateSymbol(
        "ray_node_ip_resolution",
