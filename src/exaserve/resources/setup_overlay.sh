#!/bin/bash
# Per-node script: build /tmp/exaserve_overlay/ray symlink farm.
#
...
#
# Required env:
#   EXASERVE_OVERLAY_PATCHES_DIR  /tmp/overlay_patches (the bcast'd subset)
...
PATCHES_DIR="${EXASERVE_OVERLAY_PATCHES_DIR:-/tmp/overlay_patches}"
...
# Ray install dir. An explicit EXASERVE_SYSRAY override is honored when the
# python in use doesn't ship ray (e.g. CI / standalone wheel smoke tests).
if [ -n "${EXASERVE_SYSRAY:-}" ] && [ -d "$EXASERVE_SYSRAY/serve/_private" ]; then
    SYSRAY="$EXASERVE_SYSRAY"
else
