...
def _patch_ray_serve_proxy_future_timeout() -> None:
    ...
    if wrap_as_future is None:
        return
    if getattr(wrap_as_future, "_exaserve_proxy_timeout_patch", False):
        return
...
def _install_proxy_actor_profiling_hook() -> None:
    ...
    Each proxy writes a JSON file to /tmp/exaserve_proxy_profile/ with:
    ...
    Enabled by EXASERVE_PROXY_PROFILE=1.
    """
    if os.environ.get("EXASERVE_PROXY_PROFILE") != "1":
        return
    ...
    original_import = builtins.__import__
    if getattr(original_import, "_exaserve_proxy_profile_hook", False):
        return
    ...
    _profiling_import._exaserve_proxy_profile_hook = True
...
def _wrap_proxy_actor_class(proxy_module) -> None:
    ...
    if cls is None or getattr(cls, "_exaserve_profiled", False):
        return
    ...
    def _profiled_init(self, *args, **kwargs):
        ...
        try:
            ...
            self._exaserve_profile["init_end_s"] = _time.time()
            self._exaserve_profile["init_duration_s"] = round(
                self._exaserve_profile["init_end_s"] - self._exaserve_profile["init_start_s"], 4
            )
            self._exaserve_profile["node_id"] = getattr(self, "_node_id", "unknown")

    async def _profiled_ready(self):
        if hasattr(self, "_exaserve_profile"):
            self._exaserve_profile["ready_start_s"] = _time.time()
        try:
            result = await _orig_ready(self)
        finally:
            if hasattr(self, "_exaserve_profile"):
                self._exaserve_profile["ready_end_s"] = _time.time()
                self._exaserve_profile["ready_duration_s"] = round(
                    self._exaserve_profile["ready_end_s"] - self._exaserve_profile["ready_start_s"], 4
                )
                self._exaserve_profile["total_python_s"] = round(
                    self._exaserve_profile["ready_end_s"] - _process_python_start, 4
                )
                _save_proxy_profile(self._exaserve_profile)
        return result

    cls.__init__ = _profiled_init
    cls.ready = _profiled_ready
    cls._exaserve_profiled = True
    _patch_log("Wrapped ProxyActor.__init__ and ready() for profiling")


def _save_proxy_profile(profile: dict) -> None:
    """Write proxy profile to /tmp/exaserve_proxy_profile/<hostname>_<pid>.json"""
    import json as _json
    profile_dir = "/tmp/exaserve_proxy_profile"
    try:
