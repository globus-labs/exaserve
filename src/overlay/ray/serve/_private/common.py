...
# in memory and flushes every AURORA_PROBE_FLUSH_N calls. Still writes to
# Lustre under $AURORA_RUN_LOG_DIR but with ~100× fewer open/close ops.
import atexit as _aurora_atexit
import os as _aurora_os
import socket as _aurora_socket

_AURORA_PROBE_FLUSH_N = int(_aurora_os.environ.get("AURORA_PROBE_FLUSH_N", "500"))
_aurora_probe_buf: list[tuple[float, float]] = []
_aurora_probe_path: Optional[str] = None
_aurora_probe_inited = False


def _aurora_probe_init():
    global _aurora_probe_path, _aurora_probe_inited
    _aurora_probe_inited = True
    # Write to node-local tmpfs. aurora_serve._collect_instrumentation_all
    # gathers this to Lustre at end of Stage 3 via Ray node-affinity remote
    # tasks. Keeps MDS load to ~1 open per file per node.
    inst_dir = "/tmp/aurora_inst"
    try:
        _aurora_os.makedirs(inst_dir, exist_ok=True)
        _aurora_probe_path = f"{inst_dir}/get_actor_calls_{_aurora_os.getpid()}.csv"
    except Exception:
        _aurora_probe_path = None
    _aurora_atexit.register(_aurora_probe_flush)


def _aurora_probe_flush():
    global _aurora_probe_buf
    if not _aurora_probe_path or not _aurora_probe_buf:
        return
    try:
        # One fopen/fwrite/fclose for up to FLUSH_N entries.
        with open(_aurora_probe_path, "a") as f:
            f.write("".join(f"{t:.6f},{dur*1000:.3f}\n" for (t, dur) in _aurora_probe_buf))
        _aurora_probe_buf.clear()
    except Exception:
        pass


def _aurora_probe_record(t: float, dur: float):
    if not _aurora_probe_inited:
        _aurora_probe_init()
    if _aurora_probe_path is None:
        return
    _aurora_probe_buf.append((t, dur))
    if len(_aurora_probe_buf) >= _AURORA_PROBE_FLUSH_N:
        _aurora_probe_flush()
...
class RunningReplicaInfo:
    ...
    def get_actor_handle(self) -> ActorHandle:
        ...
        # Lustre. _g_buf and _g_sink live at module level (_aurora_probe
        # helpers below) and flush every FLUSH_N calls or at process exit.
        _aurora_probe_record(_g_t0, _g_dur)
        return actor_handle
