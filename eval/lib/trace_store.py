...
def trace_store_root(root: str | None = None) -> str:
    base_root = root or os.path.join(SITE_CONFIG.experiments_root, "traces")
...
def materialize_trace_artifact(
    ...
    *,
    ...
    if not os.path.exists(trace_path):
        ...
        _write_trace(trace_path, spec, rows)
        dump_json_file(
            metadata_path,
            {
                "trace_id": trace_id,
...
def _generate_rows(spec: ExperimentSpec) -> list[dict[str, Any]]:
    if spec.trace.kind == "weak_scaling":
        return _generate_weak_scaling_rows(spec)
    if spec.trace.kind == "azure_trace":
        return _generate_azure_rows(spec)
    raise ValueError(f"Unsupported trace.kind: {spec.trace.kind}")


def _load_prompt_bank(path: str) -> list[str]:
...
def _build_tokenizer_map(spec: ExperimentSpec) -> dict[str, Any]:
    tokenizer_paths = {}
    for model in spec.deployment.models:
...
def _generate_weak_scaling_rows(spec: ExperimentSpec) -> list[dict[str, Any]]:
    prompt_bank = _load_prompt_bank(spec.trace.input_prompt_path)
    tokenizers = _build_tokenizer_map(spec)
...
def _generate_azure_rows(spec: ExperimentSpec) -> list[dict[str, Any]]:
