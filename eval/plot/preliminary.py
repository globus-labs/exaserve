...
Reads only result0.json from explicit run groups. The same run-group identity is
used for every requested spec unless a SPEC=runN override is supplied.
"""
...
EXP_ROOT = get_runs_root()


def _exact_rg(spec_name: str, run_group: str) -> Path:
    if re.fullmatch(r"run\d+", run_group) is None:
        raise ValueError(f"invalid explicit run group {run_group!r}")
    path = EXP_ROOT / spec_name / run_group
    if not path.is_dir():
        raise FileNotFoundError(f"run group does not exist: {path}")
    return path
...
def _load_variants(run_group: Path) -> list[tuple[str, int, dict]]:
    ...
    for child in sorted(run_group.iterdir()):
        ...
        if n == 0:
            continue
        result_path = child / "results" / "result0.json"
...
def plot_slo_curve(run_group: Path, out_path: Path) -> None:
    ...
    for ax, (n_nodes, pts) in zip(axes, sorted(by_node.items())):
        ...
        for preset_name in PRESETS_TO_SHOW:
            ...
            ax.plot(
                xs, ys, label=f"goodput ({preset_name})", **PRESET_STYLE[preset_name], linewidth=2
            )
    ...
    fig.suptitle("SLO-attainment curve: goodput vs offered load (slo_curve_v1)", fontsize=13)
    fig.tight_layout()
...
def plot_strong_scaling(run_group: Path, out_path: Path, target_rps: float = 500.0) -> None:
    variants = _load_variants(run_group)
    pts = []
    ...
    ax_rps.axhline(
        target_rps, color="black", linestyle="--", alpha=0.6, label=f"target {target_rps:.0f} RPS"
    )
    ax_rps.plot(xs, achieved, marker="o", linewidth=2, color="#1f77b4", label="achieved")
    ax_rps.set_xscale("log", base=2)
    ax_rps.set_xlabel("Nodes")
    ...
    ax_lat.plot(xs, [p[2] for p in pts], marker="o", color="#2ca02c", label="TTFT p50")
    ax_lat.plot(xs, [p[3] for p in pts], marker="s", color="#d62728", label="TTFT p99")
    ax_lat.plot(xs, [p[4] for p in pts], marker="^", color="#9467bd", label="E2E p99")
...
def plot_burstgpt(run_group: Path, out_path: Path) -> None:
    ...
    ax_bar.plot(xs, ach, marker="D", color="black", linestyle="", label="achieved RPS")
    ...
    ax_ttft.axhline(0.5, color="#d62728", linestyle=":", alpha=0.5, label="interactive SLO (0.5s)")
    ax_ttft.axhline(2.0, color="#1f77b4", linestyle=":", alpha=0.5, label="mlperf_8b SLO (2s)")
    ax_ttft.set_xscale("log", base=2)
    ...
    fig.suptitle("BurstGPT (peak window) — bursty workload (burstgpt_v1)", fontsize=13)
    fig.tight_layout()
    fig.savefig(out_path, dpi=120)
    plt.close(fig)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--run-group",
        required=True,
        help="Exact runN identity used for every spec unless overridden",
    )
    parser.add_argument(
        "--run-group-override",
        action="append",
        default=[],
        metavar="SPEC=runN",
        help="Exact per-spec override; may be repeated",
    )
    args = parser.parse_args(argv)

    overrides = {}
    for raw in args.run_group_override:
        if "=" not in raw:
            parser.error(f"invalid --run-group-override {raw!r}; expected SPEC=runN")
        spec, run_group = raw.split("=", 1)
        if not spec or re.fullmatch(r"run\d+", run_group) is None:
            parser.error(f"invalid --run-group-override {raw!r}; expected SPEC=runN")
        if spec in overrides:
            parser.error(f"duplicate run-group override for {spec!r}")
        overrides[spec] = run_group
    ...
    for spec, fn in plans:
        try:
            rg = _exact_rg(spec, overrides.get(spec, args.run_group))
