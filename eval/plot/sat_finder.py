...
EXP = Path("/lus/flare/projects/AuroraGPT/wenyiw/data/experiments/runs")
...
def main() -> int:
    ...
    ax_t.plot(offered, p99, marker="^", color="#d62728",
              linewidth=2.5, label="TTFT p99")
    ax_t.axhline(args.ttft_slo, color="black", linestyle="--", alpha=0.5,
                 label=f"SLO ({args.ttft_slo}s)")
    ax_t.axhline(0.5, color="gray", linestyle=":", alpha=0.4,
                 label="interactive SLO (0.5s)")
    if knee_idx is not None:
        knee_x = offered[knee_idx]
        ax_t.axvline(knee_x, color="red", linestyle="--", alpha=0.6)
        ax_t.text(knee_x + 0.2, ax_t.get_ylim()[1] * 0.5,
    ...
    ax_r.plot(offered, offered, color="gray", linestyle=":",
              alpha=0.6, label="ideal (achieved = offered)")
    ax_r.plot(offered, achieved, marker="o", color="#1f77b4",
              linewidth=2, label="achieved")
    for i, (off, ach) in enumerate(zip(offered, achieved)):
        ax_r.annotate(f"{ach / off * 100:.0f}%", (off, ach),
    ...
    fig.suptitle(f"Saturation finder: {args.spec} (Llama-3-8B, 1 node, 12 replicas)",
                 fontsize=13)
    fig.tight_layout()
