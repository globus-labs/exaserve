...
def _extract_plot_template_fields(
    ...
    deployment_cfg = cfg.get("deployment_plan") or cfg.get("model_deployment_config") or {}
    replay_cfg = cfg.get("job_replay_client_config") or {}
    proxy_cfg = cfg.get("proxy_config") or {}

    model_cfgs = deployment_cfg.get("models") or deployment_cfg.get("model_configs") or []
    ...
    gateway_cfg = deployment_cfg.get("gateway") or {}
    proxy_type = str(gateway_cfg.get("kind") or proxy_cfg.get("type", "none"))
    proxy_type_label = _proxy_type_label(proxy_type)
    proxy_num_workers = gateway_cfg.get("worker_count", proxy_cfg.get("num_workers", ""))
...
def load_results(
    ...
    Args:
        results_folder: Path to a materialized run group directory.
                        Results are expected at <N-nodes>/results/result{index}.json.
        target_indices: Exact result indices to aggregate (e.g. [0, 1, 2]).
        ...
        node_select_map: Optional dict mapping node_count -> result_index.
                         ...
                         listed node. Every observed node must be listed.
    ...
    for subdir in sorted(results_path.iterdir()):
        ...
        if node_select_map is not None:
            if node_count in node_select_map:
                ...
                if not complete:
                    raise ValueError(f"[weakscaling] {fpath}: {reason}")
                ...
                print(
                    f"Loaded {subdir.name}: result{idx}.json  TPS={tps:.2f}, "
                    f"RPS={rps:.2f}, Errors={errors}"
                )
            else:
                raise ValueError(
        ...
        if target_indices is not None:
            ...
            print(
                f"Loaded {subdir.name}: {len(tps_list)} runs. TPS={tps_mean:.2f}±{tps_std:.2f}, RPS={rps_mean:.2f}±{rps_std:.2f}, Errors={errors_mean:.1f}"
            )
            results.append(
                (
                    node_count,
...
def plot_weak_scaling(
    ...
    ax1.set_ylabel(
        "Total Throughput (Tokens per Second, TPS)", color=color1, fontsize=12, fontweight="bold"
    )
    ...
    ax2.set_ylabel("Requests per Second (RPS)", color=color2, fontsize=12, fontweight="bold")
    ...
    if len(results) > 0:
        ...
        (ideal_tps_line,) = ax1.plot(
            ...
            linewidth=2,
            alpha=0.6,
            label="Ideal TPS Scaling",
            zorder=2,
        )
        (ideal_rps_line,) = ax2.plot(
            num_nodes,
            ideal_rps,
            "--",
            color=color2,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ray",
        help="Backend name for display purposes (e.g. 'ray', 'mpi'). Default: ray.",
    )
    parser.add_argument(
        "-o",
        "--output_path",
    ...
    parser.add_argument(
        ...
        "Calculates mean and error bars. Default: exact result0 only.",
    )
    parser.add_argument(
        "--exclude-nodes",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated node counts paired with --select (e.g., '1,2,4,8,16,32').",
    )
    parser.add_argument(
        "--select",
        type=str,
    ...
    if args.node_list and args.select:
        ...
        print(
            f"Per-node result selection: { {k: f'result{v}.json' for k, v in node_select_map.items()} }"
        )
    ...
    assert not missing_files  # load_results fails closed before returning
