...
`<run_group>/<N>-nodes/results/result{INDEX}.json`), compute goodput per SLO preset:
...
def _request_meets_slo(req: dict, preset: SLOPreset) -> bool:
    ...
    Skips records that errored. A record missing any component required by the
    selected preset fails that preset; absent streaming evidence must never be
    interpreted as meeting a streaming SLO.
    """
    ...
    if preset.ttft_s is not None:
        ttft = req.get("ttft_s")
        if ttft is None or ttft > preset.ttft_s:
            return False
    if preset.tpot_s is not None:
        ttft = req.get("ttft_s")
        lat = req.get("latency")
        out_tok = req.get("actual_completion_tokens") or req.get("output_len")
        if ttft is None or lat is None or not out_tok or out_tok <= 1:
            return False
    ...
    if preset.tbt_p99_s is not None:
        ...
        if tbt is None or tbt > preset.tbt_p99_s:
            return False
    return True
...
def _strip_warmup(result: dict) -> tuple[dict, int]:
    ...
    view["overall"]["rps"] = total_completed / total_duration if total_duration > 0 else 0.0
...
def _exact_result(subdir: Path, index: int) -> Optional[Path]:
    rdir = subdir / "results"
    if not rdir.is_dir():
        return None
...
def walk_run_group(
    run_group_dir: Path, index: int, excluded: Iterable[int] = ()
) -> list[tuple[int, Path]]:
    out = []
    for child in sorted(run_group_dir.iterdir()):
        ...
        if n == 0 or n in excluded:
            continue
        rfile = _exact_result(child, index)
        if rfile is None:
            raise FileNotFoundError(
                f"explicit result{index}.json is missing for run variant {child.name}"
            )
        out.append((n, rfile))
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--run-group-name",
        default=None,
        help="Exact runN identity within an experiment",
    )
    parser.add_argument(
        "--index", type=int, default=0, help="Pick exact result{INDEX}.json (default: 0)"
    )
    parser.add_argument("--exclude-nodes", type=int, nargs="*", default=[])
    parser.add_argument(
        "--preset",
        nargs="*",
        default=list(SLO_PRESETS.keys()),
        help=f"SLO presets to evaluate. Defaults to all of: {', '.join(SLO_PRESETS)}",
    )
    parser.add_argument("--csv", default=None, help="Write a long-format CSV here")
    parser.add_argument(
        "--plot", default=None, help="Save a goodput-vs-num_nodes plot at this path"
    )
    parser.add_argument(
        "--ttft-required",
        action="store_true",
    ...
    if args.run_group:
        ...
        if not args.run_group_name:
            parser.error("--experiment requires an explicit --run-group-name runN identity")
        run_group_dir = Path(resolve_run_group_dir(args.experiment, run_group=args.run_group_name))
    ...
    for n_nodes, rfile in pairs:
        ...
        complete, reason = result_is_complete(data)
        if not complete:
            parser.error(f"{rfile}: {reason}")
        if not args.include_warmup:
            data, dropped = _strip_warmup(data)
            if dropped:
                print(
                    f"  {rfile.name} ({n_nodes} nodes): dropped {dropped} "
        ...
        for preset in presets:
            ...
            rows.append(stats)
            needs_stream = (
                (preset.ttft_s is not None)
                or (preset.tpot_s is not None)
    ...
    if ttft_missing_for:
        print()
        print(
            "WARNING: TTFT/TPOT data missing for presets: "
    ...
    if tbt_missing_for:
        print()
        print(
            "WARNING: P99-TBT data missing for presets: "
