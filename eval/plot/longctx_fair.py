...
def _ttft_stats(data: dict) -> tuple[float, float, float]:
    ts = sorted([r["ttft_s"] for r in data.get("requests", [])
    ...
    for in_len, pts in sorted(by_input.items()):
        ...
        for pct_label, (ls, marker) in PCT_STYLE.items():
            ...
            ax.plot(xs, ys, linestyle=ls, marker=marker,
                    color=COLORS[in_len], alpha=0.9 if pct_label == "p99" else 0.55,
        ...
        for n, (_p50, _p95, p99), achieved in pts:
            ax.annotate(f"{achieved:.0f} rps", (n, p99), xytext=(4, 4),
...
def main() -> int:
    ...
    if eqsat:
        ...
        _plot_panel(ax1, eqrate, "Equal offered rate (4 rps/N)",
                    "Both lengths lightly loaded -> TTFT = prefill cost only")
        _plot_panel(ax2, eqsat, "Equal saturation (~80% per length)",
    ...
    fig.suptitle("Long-context v2: fair TTFT comparison "
