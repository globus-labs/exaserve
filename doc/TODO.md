...
[] Eval/benchmark harness is still PBS-only (eval/lib/schedulers/pbs.py render + eval/lib/run_executor qsub/qstat/submit_all queue limits + eval/lib/backends/ray.py). Add a Slurm path there if scaling benchmarks are needed off-Aurora (separate lift from the package deploy path).
